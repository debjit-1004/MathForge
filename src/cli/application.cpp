...
namespace mathforge::cli
{
    ...
    void Application::setupLogging()
    {
        ...
        if (options_.logLevel == "trace")
            level = core::LogLevel::TRACE;
        else if (options_.logLevel == "debug")
            level = core::LogLevel::DEBUG;
        else if (options_.logLevel == "info")
            level = core::LogLevel::INFO;
        else if (options_.logLevel == "warn")
            level = core::LogLevel::WARN;
        else if (options_.logLevel == "error")
            level = core::LogLevel::ERROR;
        else if (options_.logLevel == "fatal")
            level = core::LogLevel::FATAL;
    ...
    int Application::processFile()
    {
        try
        {
            ...
            content = mathforge::core::normalizeUnicodeMath(content);
        ...
        catch (const core::LexicalException &e)
        {
            ...
            std::cerr << "ERROR: Lexical error at line " << e.getLine() << ", column " << e.getColumn() << ": " << e.what() << std::endl;
            return 10;
        }
        catch (const core::SyntaxException &e)
        {
            ...
            //          ", column " + std::to_string(e.getColumn()) + ": " + e.what());
            std::cerr << "ERROR: Syntax error at line " << e.getLine() << ", column " << e.getColumn() << ": " << e.what() << std::endl;
            return 11;
        }
